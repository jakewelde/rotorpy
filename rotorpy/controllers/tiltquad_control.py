...
class SE3TiltControl(object):
    ...
    def control_allocation_slidyquad(self, F_x, F_y, F_z, T_x, T_y, T_z):
        print(self.k_eta)
        print(self.rotor_speed_max)
        self.control_allocation.solve_prob([F_x, F_y, F_z], [T_x, T_y, T_z])
        f = self.control_allocation.res.x
        # self.control_allocation.plot_forces()
        results = (f"F1_x: {f[0]}, F1_y: {f[1]}, F1_z: {f[2]}\n"
                   f"F2_x: {f[3]}, F2_y: {f[4]}, F2_z: {f[5]}\n"
                   f"F3_x: {f[6]}, F3_y: {f[7]}, F3_z: {f[8]}\n"
                   f"F4_x: {f[9]}, F4_y: {f[10]}, F4_z: {f[11]}")
