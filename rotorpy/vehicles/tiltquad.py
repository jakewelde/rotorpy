...
class TiltQuad(object):
    ...
    def __init__(self, quad_params, aero=False, initial_state={'x': np.array([0, 0, 0]),
                                                              'v': np.zeros(3, ),
    ...
    def _s_dot_fn(self, t, s, cmd_rotor_thrusts):
        ...
        (FtotB, MtotB) = self.compute_body_wrench(state['w'], cmd_rotor_thrusts, body_airspeed_vector)
    ...
    def compute_body_wrench(self, body_rates, rotor_thrusts, body_airspeed_vector):
        ...
        # M_yaw = self.rotor_dir * (
                    # np.array([0, 0, self.k_m / self.k_eta])[:, np.newaxis] * np.linalg.norm(rotor_thrusts, axis=1))
        M_yaw = self.rotor_dir * (np.array([0, 0, self.k_m/self.k_eta])[:, np.newaxis] * rotor_thrusts[:, 2])
    ...
    def _unpack_state(self, s):
        ...
        """
        state = {'x': s[0:3], 'v': s[3:6], 'q': s[6:10], 'w': s[10:13], 'wind': s[13:16],
                 'rotor_thrusts': s[16:].reshape(4, 3)}
        return state
...
if __name__ == "__main__":
    ...
    initial_state = {'x': np.zeros(3, ), 'v': np.zeros(3, ), 'q': np.array([0, 0, 0, 1]), 'wind': np.zeros(3, ),
                     'w': np.zeros(3, ), 'rotor_thrusts': control['cmd_motor_thrusts']}
