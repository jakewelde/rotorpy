...
class Plotter():
    ...
    def plot_results(self, plot_mocap, plot_estimator, plot_imu):
        ...
        num_rotors = self.rotor_thrust_vecs.shape[0]
        num_subplots = int(np.sqrt(num_rotors))
        fig, axes = plt.subplots(nrows=num_subplots, ncols=num_subplots, num="Rotor Forces")
        for i in range(num_rotors):
            ax = axes.ravel()[i]
            ax.plot(time, self.rotor_thrust_vecs[:, i, 0], 'r.', label='X')
            ax.plot(time, self.rotor_thrust_vecs[:, i, 1], 'g.', label='Y')
            ax.plot(time, self.rotor_thrust_vecs[:, i, 2], 'b.', label='Z')
            ax.plot(time, self.rotor_thrust_vecs_des[:, i, 0], 'r', label='X')
            ax.plot(time, self.rotor_thrust_vecs_des[:, i, 1], 'g', label='Y')
            ax.plot(time, self.rotor_thrust_vecs_des[:, i, 2], 'b', label='Z')
            ax.set_xlabel("time, s")
            ax.set_ylabel("thrust, N")
